...
with open(os.path.join(OUT, "softmax.txt"), "w") as f:
    ...
    f.write("row0 = all 0 (uniform)\n")
    f.write("row1 = all 1000 (overflow if the max is not subtracted)\n")
    f.write("row2 = one 1000 and the rest normal (nearly one-hot)\n")
    f.write("row3 = all -1000 (underflow if the max is not subtracted)\n")
    f.write("row4,row5 = normal(0, 2) with seed 0\n")
    f.write("logsoftmax_out.bin float32 rows x cols\n")
    f.write(f"\nwide_rows {WIDE_ROWS}\nwide_cols {WIDE_COLS}\n")
    f.write("wide_*.bin: cols > TPB, to exercise the loop `i += TPB`\n")
    f.write("wide row0 = uniform; row1 = the max in the LAST column;\n")
    f.write("wide row2 = all -800; row3 = normal(0,2)\n")

print("softmax golden written to", OUT)
